...
def greedy_cow_transport(cows,limit=10):
    ...
    while len(cowsSorted) > 0:
        currentTrip = []
        cowsSortedCopy = cowsSorted[:]
        for cow in range(len(cowsSortedCopy)):
            if cowsSortedCopy[cow][1] + weight <= limit:
                currentTrip.append(cowsSortedCopy[cow][0])
                weight += cowsSortedCopy[cow][1]
                cowsSorted.remove(cowsSortedCopy[cow])
...
def brute_force_cow_transport(cows,limit=10):
    ...
    for trip in get_partitions(cows):
        validTrip = True
        for names in trip:
            totalWeight = sum(map(lambda x: cows[x], names))
            if totalWeight > limit:
                validTrip = False
                break
        if validTrip and (bestTrip is None or len(trip) < bestTripLength):
            bestTrip = trip
            bestTripLength = len(trip)
    return bestTrip
