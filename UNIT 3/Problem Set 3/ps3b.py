...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        prob2 = random.random()
        if prob2 <= self.maxBirthProb * (1 - self.popDensity):
...
class Patient(object):
    ...
    def update(self):
        ...
        for w in virusesCopy2:
            try:
                w.reproduce(popDensity)
                self.viruses.append(w)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for t in range(numTrials):
        ...
        data = data + virusCount
    dataAvg = data/numTrials
    pylab.plot(list(dataAvg), label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if len(counter) == len(activeDrugs) and random.random() <= self.maxBirthProb * (1 - popDensity):
                if self.mutProb*len(self.resistances) ==\
                    len(self.resistances)//2 and (len(self.resistances) > 1):
                        return ResistantVirus(self.maxBirthProb, self.clearProb,
                                              {key:self.resistances[key] for i, key in enumerate(self.resistances) if i%2 ++ 0},
                                              self.mutProb)
                if random.random() <= self.mutProb:
                    self.resistances.update((k, False) if self.resistances[k] == True else (k, True) for k, 
                                            v in self.resistances.items())
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for t in range(numTrials):
        ...
        for k in range(150):
            ...
            virusCount.append(patient.getTotalPop())
            resistVirusCount.append(patient.getResistPop(['guttagonol']))
        data1 = data1 + virusCount
        data2 = data2 + resistVirusCount
    data1avg = data1/numTrials
    data2avg = data2/numTrials
    pylab.plot(list([float('{0:.1f}'.format(i)) for i in data1avg]), label = r'Non-resistant population')
    pylab.plot(list([float('{0:.1f}'.format(j)) for j in data2avg]), label = r'Guttagonol Resistant population')
